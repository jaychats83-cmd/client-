...
public class zznwpi extends ModuleStructure implements IMinecraft {

    private static zznwpi instance;

    public static zznwpi getInstance() {
        return instance;
    }
    ...
    SelectSetting mode = new SelectSetting(StringHelper.decrypt(new byte[]{7, (byte)-84, 59, (byte)-115}), StringHelper.decrypt(new byte[]{48, (byte)-71, (byte)-113, 85, 101, (byte)-21, 94, (byte)-10, 62, (byte)-70, 47, (byte)-115}))
            .value(StringHelper.decrypt(new byte[]{24, (byte)-85, 48, (byte)-123, 112}), StringHelper.decrypt(new byte[]{13, (byte)-85, 48, (byte)-101, 102}), StringHelper.decrypt(new byte[]{9, (byte)-85, 62, (byte)-127, 124}), StringHelper.decrypt(new byte[]{9, (byte)-79, 38, (byte)-101, 102, (byte)-6, 91, (byte)-91}), StringHelper.decrypt(new byte[]{9, (byte)-86, 45, (byte)-117, 126, (byte)-2}))
            .selected(StringHelper.decrypt(new byte[]{24, (byte)-85, 48, (byte)-123, 112}));

    SliderSettings crystalRotationSpeed = new SliderSettings(StringHelper.decrypt(new byte[]{9, (byte)-79, 38, (byte)-101, 102, (byte)-6, 91, (byte)-10, 24, (byte)-84, 43, (byte)-119, 102, (byte)-14, 88, (byte)-72, 106, (byte)-112, 47, (byte)-115, 119, (byte)-1}), StringHelper.decrypt(new byte[]{9, (byte)-79, 38, (byte)-101, 102, (byte)-6, 91, (byte)-10, 56, (byte)-84, 43, (byte)-119, 102, (byte)-14, 88, (byte)-72, 106, (byte)-80, 47, (byte)-115, 119, (byte)-1}))
            .range(0.1f, 2.0f)
            .visible(() -> mode.isSelected("Crystals"));

    ColorSetting color1 = new ColorSetting("Color 1", "First gradient color")
            .setColor(new Color(255, 101, 57, 255).getRGB());

    ColorSetting color2 = new ColorSetting("Color 2", "Second gradient color")
            .setColor(new Color(255, 50, 150, 255).getRGB());

    ColorSetting color3 = new ColorSetting("Color 3", "Third color for Ghost")
    ...
    public zznwpi() {
        super(StringHelper.decrypt(new byte[]{30, (byte)-94, 45, (byte)-113, 119, (byte)-17, 114, (byte)-91, 58}), StringHelper.decrypt(new byte[]{30, (byte)-94, 45, (byte)-113, 119, (byte)-17, 23, (byte)-109, 57, (byte)-77}), ModuleCategory.ESP);
        instance = this;
    ...
    private void renderChain(MatrixStack stack, VertexConsumerProvider provider, LivingEntity target, float alpha,
        ...
        VertexConsumer consumer = provider
                .getBuffer(ClientPipelines.CHAIN_ESP.apply(Identifier.of("starry", "images/world/chain.png")));
    ...
    private void renderRhomb(MatrixStack stack, VertexConsumerProvider provider, LivingEntity target, float alpha) {
        VertexConsumer consumer = provider
                .getBuffer(ClientPipelines.ROMB_ESP.apply(Identifier.of("starry", "images/world/cube.png")));
    ...
    private void renderGhost(MatrixStack stack, VertexConsumerProvider consumers, LivingEntity target, float alpha) {
        VertexConsumer consumer = consumers
                .getBuffer(ClientPipelines.GHOSTS_ESP.apply(Identifier.of("starry", "images/particle/ghost-glow.png")));
    ...
    private class Crystal {
        ...
        private void drawBloomEffect(MatrixStack stack, VertexConsumerProvider provider, int baseColor, float anim) {
            VertexConsumer bloomConsumer = provider
                    .getBuffer(ClientPipelines.BLOOM_ESP.apply(Identifier.of("starry", "images/particle/glow.png")));
