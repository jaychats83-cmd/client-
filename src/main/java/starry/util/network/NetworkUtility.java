...
import starry.mixin.ClientConnectionAccessor;
import starry.mixin.IClientWorld;
...
public class NetworkUtility implements IMinecraft {
    ...
    public void sendUse(Hand hand) {
        try (PendingUpdateManager pendingUpdateManager = ((IClientWorld)mc.world).client$pending().incrementSequence()) {
            ...
            PlayerInteractItemC2SPacket packet = new PlayerInteractItemC2SPacket(hand, i, mc.player.getYaw(), mc.player.getPitch());
