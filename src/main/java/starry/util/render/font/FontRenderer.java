...
public class FontRenderer {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger("starry/FontRenderer");
    ...
    public void loadFont(String name, String path) {
        Identifier jsonId = Identifier.of("starry", "fonts/" + path + ".json");
        Identifier textureId = Identifier.of("starry", "fonts/" + path + ".png");
    ...
    public void initialize() {
        ...
        FontAtlas fallback = fonts.get("test");
        if (fallback != null) {
            pipeline.setFallbackAtlas(fallback);
            LOGGER.info("Font fallback set to 'test' atlas ({} glyphs)", fallback.getGlyphCount());
        }
        initialized = true;
