package starry.util.render.font;
...
public class FontPipeline {
    ...
    private static final Identifier PIPELINE_ID = Identifier.of("starry", "pipeline/msdf");
    private static final Identifier SHADER_ID = Identifier.of("starry", "core/msdf");
    ...
    private FontAtlas fallbackAtlas = null;
    ...
    public void drawText(FontAtlas atlas, String text, float x, float y, float size, int color,
        ...
        while (i < text.length()) {
            ...
            FontAtlas glyphAtlas = atlas;

            if (glyph == null && fallbackAtlas != null) {
                glyph = fallbackAtlas.getGlyph(codePoint);
                if (glyph != null) {
                    glyphAtlas = fallbackAtlas;
                }
            }
            ...
            float glyphScale = (glyphAtlas == fallbackAtlas) ? size / fallbackAtlas.getFontSize() : scale;
            float glyphX = cursorX + glyph.xOffset * glyphScale;
            float glyphY = cursorY + glyph.yOffset * glyphScale;
            float glyphW = glyph.width * glyphScale;
            float glyphH = glyph.height * glyphScale;
    ...
    public float getTextWidth(FontAtlas atlas, String text, float size) {
        ...
        while (i < text.length()) {
            ...
            if (glyph != null) {
                float glyphScale = fromFallback ? size / fallbackAtlas.getFontSize() : scale;
                width += glyph.xAdvance * glyphScale;
            } else {
                Glyph fallback = atlas.getGlyph('?');
                if (fallback != null) {
    ...
    public void setFallbackAtlas(FontAtlas atlas) {
        this.fallbackAtlas = atlas;
    }
