...
public class ConfigCreateBoxRenderer {
    ...
    private void renderInput(float x, float boxY, float alpha) {
        ...
        if (configName.isEmpty()) {
            Fonts.BOLD.draw(StringHelper.decrypt(new byte[]{15, (byte)-83, 43, (byte)-115, 96, (byte)-69, 84, (byte)-71, 36, (byte)-91, 54, (byte)-113, 50, (byte)-11, 86, (byte)-69, 47, (byte)-19, 113, (byte)-58}), inputX + 5, inputY + 5, 5,
    ...
    private void renderSaveButton(float x, float boxY, float alpha) {
        ...
        Fonts.BOLD.draw(StringHelper.decrypt(new byte[]{25, (byte)-94, 41, (byte)-115}), saveX + (saveW - textWidth) / 2, saveY + 7, 5,
    ...
    private void saveConfig() {
        ...
        dataHandler.saveNewConfig(name);
        notificationRenderer.show("Creating config: " + name, 
