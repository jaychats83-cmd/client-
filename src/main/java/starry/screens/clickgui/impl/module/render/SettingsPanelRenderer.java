package starry.screens.clickgui.impl.module.render;

import net.minecraft.client.gui.DrawContext;
import org.lwjgl.glfw.GLFW;
import starry.modules.module.ModuleStructure;
import starry.screens.clickgui.impl.module.handler.ModuleAnimationHandler;
...
public class SettingsPanelRenderer {
    ...
    private static final float HEADER_BIND_BOX_WIDTH = 38f;
    private static final float HEADER_BIND_BOX_HEIGHT = 10f;

    private final ModuleAnimationHandler animationHandler;
    private float headerBindBoxX, headerBindBoxY;

    public SettingsPanelRenderer(ModuleAnimationHandler animationHandler) {
        this.animationHandler = animationHandler;
    }

    public void render(DrawContext context, ModuleStructure selectedModule, ModuleStructure bindingModule,
                       ...
                       int guiScale, float alphaMultiplier, ModuleScrollHandler scrollHandler,
                       ModuleAnimationHandler animHandler, ModuleBindHandler bindHandler) {
        ...
        if (desc != null && !desc.isEmpty()) {
            Fonts.BOLD.draw(desc.length() > 52 ? desc.substring(0, Math.min(desc.length(), 52)) + "..." : desc, x + 15, y + 20, 5, new Color(128, 128, 128, (int) (150 * alphaMultiplier)).getRGB());
            Fonts.GUI_ICONS.draw("C", x + 8, y + 20, 6, new Color(128, 128, 128, (int) (150 * alphaMultiplier)).getRGB());
        }

        boolean headerBindActive = bindingModule == selectedModule;
        int headerBindKey = selectedModule.getKey();
        String headerBindText = headerBindActive ? "..." : bindHandler.getBindDisplayName(headerBindKey);

        float headerBindTextWidth = Fonts.BOLD.getWidth(headerBindText, 5);
        float headerBindBoxWidthActual = Math.max(HEADER_BIND_BOX_WIDTH, headerBindTextWidth + 12);
        headerBindBoxX = x + width - 8 - headerBindBoxWidthActual;
        headerBindBoxY = y + 5;

        int headerBgAlpha = (int) (30 * alphaMultiplier);
        Render2D.rect(headerBindBoxX, headerBindBoxY, headerBindBoxWidthActual, HEADER_BIND_BOX_HEIGHT,
                new Color(50, 50, 55, headerBgAlpha).getRGB(), 3f);

        int headerOutlineAlpha = (int) (60 * alphaMultiplier);
        Render2D.outline(headerBindBoxX, headerBindBoxY, headerBindBoxWidthActual, HEADER_BIND_BOX_HEIGHT,
                0.5f, new Color(80, 80, 85, headerOutlineAlpha).getRGB(), 3f);

        int headerTextAlpha = (int) (160 * alphaMultiplier);
        float headerTextX = headerBindBoxX + (headerBindBoxWidthActual - headerBindTextWidth) / 2f;
        float headerTextY = headerBindBoxY + (HEADER_BIND_BOX_HEIGHT - 5) / 2f;
        Fonts.BOLD.draw(headerBindText, headerTextX, headerTextY, 5,
                new Color(140, 140, 145, headerTextAlpha).getRGB());
    ...
    public boolean isHeaderBindBoxClicked(float mouseX, float mouseY) {
        return mouseX >= headerBindBoxX && mouseX <= headerBindBoxX + HEADER_BIND_BOX_WIDTH &&
                mouseY >= headerBindBoxY && mouseY <= headerBindBoxY + HEADER_BIND_BOX_HEIGHT;
    }
